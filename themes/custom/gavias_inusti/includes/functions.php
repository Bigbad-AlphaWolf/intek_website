<?php
function gavias_inusti_render_block($key) {
...
function gavias_inusti_makeid($length = 5){
...
function gavias_inusti_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_inusti');
  return $base_url . '/' . $theme_path . '/';
}

global $gva_node_index;
function gavias_inusti_preprocess_node(&$variables) {
  global $gva_node_index;
  ...
  if ($variables['node']->getType() == 'article') {
      ...
      $variables['theme_uri'] = base_path() . drupal_get_path('theme', 'gavias_inusti');

  }
}

function gavias_inusti_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function gavias_inusti_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function gavias_inusti_preprocess_breadcrumb(&$variables){
...
function gavias_inusti_print_icon($file, $alt){
